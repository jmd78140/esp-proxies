...
public class StandardRequestHandler
{
    ...
    /**
     * Handles the request by applying service-specific customizations and executing it
     * through the configured WebClient, with support for circuit breaking and rate limiting.
     *
     * @param retargetedRequest the {@link ServerHttpRequest} containing the request details.
     * @param requestBody       the body of the HTTP request.
     * @param circuitBreaker    the {@link CircuitBreaker} applied to the request execution.
     * @param rateLimiter       the {@link RateLimiter} applied to the request execution.
    ...
    public Flux<?> handle(final ServerHttpRequest retargetedRequest, final String requestBody,
        ...
        return serviceHandler.handleRequest(retargetedRequest, requestBody)
                ...
                .map(response -> {
                    ...
                    return buildResponseWithMetrics(retargetedRequest, requestBody,
                            (ResponseEntity<String>) response, serviceHandler);
                })
    ...
    /**
     * Executes an HTTP request using the provided retargeted request and returns the response
     * as a {@link Mono} containing a {@link ResponseEntity} with a String body.
     *
     * @param retargetedRequest the {@link ServerHttpRequest} containing the request details.
     * @param requestBody       the body of the HTTP request.
     * @return a {@link Mono} containing the response entity with enriched headers and body.
     */
    public Mono<ResponseEntity<String>> executeRequest(final ServerHttpRequest retargetedRequest, final String requestBody) {
        ...
        return requestBuilder.build()
                ...
                .flatMap(responseEntity -> {
                    ...
                    technicalMetrics.addMetric("ResponseTime", new ResponseTime(requestStartTime, requestEndTime));
     ...
     *
     * @param request   the {@link ServerHttpRequest} containing the request details.
     * @param requestBody the body of the HTTP request.
     * @param response  the {@link ResponseEntity} containing the response details.
     * @param handler   the {@link IUsageMetricsSupplier} used to compute usage metrics.
     * @return a {@link ResponseEntity} with updated headers containing usage metrics.
     */
    private ResponseEntity<String> buildResponseWithMetrics(final ServerHttpRequest request,
                                                            final String requestBody,
                                                            final ResponseEntity<String> response,
                                                            IUsageMetricsSupplier handler) {
        ...
        modifiedHeaders.add(X_USAGEMETRICS, jsonUsageMetrics);
